...
描述会按 provider、读图模型和不可变 attachment ID 缓存。因此在 **CLIProxyAPI (auto)** 分组内，
带图片的会话也能切换到该分组的文本模型，全程不修改 DSH 本体。

DSH 本体的 **DeepSeek** provider 不属于本插件管理的 route。带图片的会话切换到本体文本模型时，
DSH 仍会按自身规则拒绝；插件不会修改、包装或重新标注本体模型。
